    ...
    <div class="form-fill-wrapper">
        ...
        <div class="form-fill-header">
            ...
            <div class="form-fill-header-text">
                ...
                <div class="form-fill-meta">
                    ...
                    <?php if (empty($data['existing_response']) && empty($data['admin_view'])): ?>
                        <span><i class="fa-solid fa-asterisk" style="font-size:.6rem;color:#fca5a5;"></i> Campos obrigatórios</span>
                    <?php endif; ?>
                    <?php if (!empty($data['existing_response'])): ?>
                        <span style="color:#86efac;"><i class="fa-solid fa-circle-check"></i> Já respondido</span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="form-fill-body">
            ...
            <?php if (!empty($data['existing_response']) && empty($data['admin_view'])): ?>
                ...
                <?php $resp = $data['existing_response']; ?>

                <!-- Aviso de resposta existente -->
                <div class="already-answered-box">
                    <div class="already-answered-icon">
                        <i class="fa-solid fa-circle-check"></i>
                    </div>
                    <div class="already-answered-content">
                        <h4>Já submeteu uma resposta</h4>
                        <p>
                            Respondeu a este formulário em
                            <strong><?php echo date('d/m/Y \à\s H:i', strtotime($resp->submitted_at)); ?></strong>.
                        </p>
                        <p class="mb-0" style="font-size:.85rem;color:var(--black-500);">
                            Para preencher novamente, elimine a resposta anterior clicando no botão abaixo.
                            A sua resposta anterior ficará arquivada.
                        </p>
                    </div>
                </div>
                ...
                <div class="d-flex gap-3 flex-wrap mt-4">
                    <!-- Ver resposta actual -->
                    <a href="<?php echo URLROOT; ?>/my/history/<?php echo $resp->id; ?>"
                ...
                <?php if (!empty($data['previous_answers'])): ?>
                <div class="mt-4">
                    <div class="section-title"><i class="fa-solid fa-history"></i> A sua resposta anterior</div>
                    <?php foreach ($data['previous_answers'] as $i => $answer): ?>
                    <div class="qfill-block mb-2" style="opacity:.7;">
                        <div class="qfill-label">
                            <span class="qfill-num"><?php echo $i + 1; ?></span>
                            <span class="qfill-text"><?php echo htmlspecialchars($answer->question_label ?? ''); ?></span>
                        </div>
                        <div class="ps-4 text-muted" style="font-size:.875rem;">
                            <?php if ($answer->question_type == 'checkbox'): ?>
                                <?php $vals = json_decode($answer->value ?? '[]', true); ?>
            ...
            <?php elseif (empty($data['questions'])): ?>
            <!-- ── SEM PERGUNTAS ──────────────────────────────────── -->
            ...
            <!-- ── FORMULÁRIO PARA PREENCHER ──────────────────────── -->
                ...
                <div class="df-alert df-alert-success mb-4">
                    <i class="fa-solid fa-check-circle"></i>
                    <span>Resposta anterior eliminada. Pode agora preencher o formulário novamente.</span>
                </div>
                <?php endif; ?>
                    ...
                    <div class="qfill-block" id="qblock-<?php echo $question->id; ?>">
                        ...
                        <?php elseif ($question->type === 'date'): ?>
                            ...
                            <?php if (!empty($qConfig->date_min) || !empty($qConfig->date_max)): ?>
                                <div class="form-text mt-1"><i class="fa-solid fa-circle-info me-1"></i>
                                <?php
                        ...
                        <?php elseif ($question->type === 'upload'): ?>
                            ...
                            <label class="qfill-upload-zone" for="question_<?php echo $question->id; ?>">
                                ...
                                <input type="file" class="d-none"
                                    id="question_<?php echo $question->id; ?>"
                                    name="question_<?php echo $question->id; ?>"
                                    accept="<?php echo implode(',', array_map(function($t){ return '.'.$t; }, $types)); ?>"
                                    <?php echo $question->is_required ? 'required' : ''; ?>
...
function showUploadFile(input) {
    var id  = input.id.replace('question_', '');
    var box = document.getElementById('fname-' + id);
    if (!box) return;
    if (input.files && input.files[0]) {
        var f = input.files[0];
        box.querySelector('span').textContent = f.name + ' (' + (f.size/1024).toFixed(0) + ' KB)';
        box.classList.remove('d-none');
        var zone = input.closest('.qfill-upload-zone');
        if (zone) zone.classList.add('has-file');
    }
}
function clearUpload(id) {
    var input = document.getElementById('question_' + id);
    var box   = document.getElementById('fname-' + id);
    if (input) input.value = '';
    if (box)   box.classList.add('d-none');
    var zone = input && input.closest ? input.closest('.qfill-upload-zone') : null;
    if (zone) zone.classList.remove('has-file');
}
function confirmDeleteOwn() {
    if (confirm('Tem a certeza que quer eliminar a sua resposta anterior?\n\nPoderá preencher o formulário novamente após confirmar.')) {
        document.getElementById('deleteOwnForm').submit();
    }
}
</script>
